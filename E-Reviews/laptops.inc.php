...
function setComments($conn) {
	if(isset($_POST['commentSubmit'])) {
		...
		$sql = "INSERT INTO laptops (uid, date, message) VALUES ('$uid', '$date', '$message')";
	...
	while ($row = $result->fetch_assoc()) {
		...
		if($row2 = $result2->fetch_assoc()) {
			...
			if(isset($_SESSION['id'])){
				...
				<form class='edit-form' method='POST' action='editlappy.php'>
					<input type='hidden' name='cid' value='".$row['cid']."'>
...
function editlappy($conn) {
	if(isset($_POST['commentSubmit'])) {
		...
		$sql = "UPDATE laptops SET message='$message' WHERE cid='$cid'";
		$result = $conn->query($sql);
...
function deleteComments($conn) {
		...
		$sql = "DELETE FROM laptops WHERE cid='$cid'";
		$result = $conn->query($sql);
		header("Location: msi.php");
		}
}
